...
# Step-by-step filtering as per your flowchart
data0 <- nhanes_all
...
data1 <- data0 %>%
  filter(RIDAGEYR >= 20)
stage_n(data1, "1. Age ≥ 20")

# 2. Exclude pregnant women age 20–44 (RIDEXPRG == 1)
data2 <- data1 %>%
...
data3 <- data2 %>%
  filter(!is.na(BMXBMI))
stage_n(data3, "3. Non‑missing BMI")
...
write.csv(nhanes_data, "nhanes_2017_2018_cleaned.csv", row.names = FALSE)
